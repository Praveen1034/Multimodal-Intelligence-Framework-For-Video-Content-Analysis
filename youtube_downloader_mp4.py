...
DEFAULT_OUTPUT_TEMPLATE = os.path.join(DEFAULT_DOWNLOADS_DIR, "%(title)s.%(ext)s")
MAX_RETRIES = 3
...
def check_ffmpeg_installed():
    ...
    if shutil.which("ffmpeg") is None:
        logger.error("FFmpeg is not installed. Please install FFmpeg for merging audio and video.")
        return False
    return True
...
def download_youtube_video(video_url: str, output_template: str = DEFAULT_OUTPUT_TEMPLATE) -> YouTubeDownloadResponse:
    ...
    """
    logger.debug(f"Starting download: {video_url}")
    ...
    os.makedirs(os.path.dirname(request.output_template), exist_ok=True)
    ...
    ydl_opts = {
        ...
        'format': 'bv*+ba/b',  # Selects best video + best audio separately / Both
        'merge_output_format': 'mp4',  # Changed to MP4
        'ignoreerrors': True,
        'postprocessors': [{
            'key': 'FFmpegVideoConvertor',
            'preferedformat': 'mp4'  # Changed to MP4
        }],
        'retries': 10,  # Increase retries for unstable networks
        'socket_timeout': TIMEOUT_SECONDS,  # Extend timeout
        'noprogress': False,
    }
        ...
        try:
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                if downloaded_files:
                    ...
                    logger.warning("Download incomplete. Retrying...")
...
if __name__ == "__main__":
    ...
    output_template = input("Enter output path (or press Enter for default): ").strip() or DEFAULT_OUTPUT_TEMPLATE
